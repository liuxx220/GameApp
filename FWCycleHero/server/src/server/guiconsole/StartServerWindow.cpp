...
void CStartServerWindow::OnBnClickedButton2()
{
	...
	UNORDERED_MAP< std::string, std::vector<CStartServerWindow::LAYOUT_ITEM> >::iterator iter = 
		layouts_.find(cs);
	...
	for(; iter1 != iter->second.end(); iter1++)
	{
		...
		KBEngine::EndPoint* endpoint = new KBEngine::EndPoint();
		
		KBEngine::u_int32_t address;
		endpoint->convertAddress(vec[0].c_str(), address);
		KBEngine::Address addr(address, htons(atoi(vec[1].c_str())));
...
void CStartServerWindow::saveLayouts()
{
	...
	int i = 0;
	UNORDERED_MAP< std::string, std::vector<LAYOUT_ITEM> >::iterator iter = layouts_.begin();
...
void CStartServerWindow::OnCbnSelchangeCombo3()
{
	...
	char* cs = KBEngine::strutil::wchar2char(s.GetBuffer(0));
	UNORDERED_MAP< std::string, std::vector<LAYOUT_ITEM> >::iterator iter = layouts_.find(cs);
	free(cs);
