...
public:
	...
	virtual bool process()
	{
		...
		while(true)
		{
			...
			BundleBroadcast bhandler(dlg->NetSession(), nport);
			...
			MachineInterface::onFindInterfaceAddrArgs7::staticAddToBundle(bhandler, getUserUID(), getUsername(), 
				dlg->componentType(), dlg->componentID(), (COMPONENT_TYPE)findComponentType, dlg->NetSession().intaddr().ip, 
...
void CguiconsoleDlg::commitPythonCommand(CString strCommand)
{
	...
	*/
	...
	Channel* pChannel = _NetSession.findChannel(this->getTreeItemAddr(m_tree.GetSelectedItem()));
	if(pChannel)
	{
		Bundle bundle;
		...
		bundle.send(this->NetSession(), pChannel);
...
void CguiconsoleDlg::OnTimer(UINT_PTR nIDEvent)
{
	...
	case 3:
		{
			const NetSession::ChannelMap& channels = _NetSession.channels();
			NetSession::ChannelMap::const_iterator iter = channels.begin();
			for(; iter != channels.end(); iter++)
			{
				Channel* pChannel = const_cast<KBEngine::Channel*>(iter->second);
				Bundle bundle;
				...
				bundle << _componentID;
				bundle.send(NetSession(), pChannel);
...
void CguiconsoleDlg::onExecScriptCommandCB(Channel* pChannel, std::string& command)
{
...
void CguiconsoleDlg::reqQueryWatcher(std::string paths)
{
	...
	Address addr = getTreeItemAddr(m_tree.GetSelectedItem());

	Channel* pChannel = _NetSession.findChannel(addr);
	...
	if(pChannel)
	{
		...
		bundle << paths;
		bundle.send(_NetSession, pChannel);
	}
}
...
Address CguiconsoleDlg::getTreeItemAddr(HTREEITEM hItem)
{
	if(hItem == NULL)
		return Address::NONE;
	...
	KBEngine::Address addr(address, htons(atoi(sport.c_str())));
	return addr;
}
...
void CguiconsoleDlg::connectTo()
{
	...
	KBEngine::Address addr = getTreeItemAddr(hItem);
	if(addr.ip == 0)
	{
	...
	Channel* pChannel = _NetSession.findChannel(endpoint->addr());
	if(pChannel)
	{
		_NetSession.deregisterChannel(pChannel);
		pChannel->destroy();
	}

	pChannel = new Channel(_NetSession, endpoint, Channel::INTERNAL);
	pChannel->proxyID(getTreeItemComponent(m_tree.GetSelectedItem()));
	if(!_NetSession.registerChannel(pChannel))
	{
		CString err;
...
{
	_NetSession.deregisterAllChannels();
...
void CguiconsoleDlg::OnNMClickTree1(NMHDR *pNMHDR, LRESULT *pResult)
{
	...
	*pResult = 0;
	...
	Address currAddr = this->getTreeItemAddr(hItem);
	if(currAddr.ip == 0)
		return;
...
void CguiconsoleDlg::OnToolBar_StartServer()
{
	...
	while(1)
	{
		srand(KBEngine::getSystemTime());
		uint16 nport = KBE_PORT_START + (rand() % 1000);
		BundleBroadcast bhandler(_NetSession, nport);
		...
		uint32 ip = _NetSession.intaddr().ip;
