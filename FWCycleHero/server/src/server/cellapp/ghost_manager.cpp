...
GhostManager::~GhostManager()
{
	std::map<COMPONENT_ID, std::vector< Bundle* > >::iterator iter = messages_.begin();
	for(; iter != messages_.end(); ++iter)
	{
		std::vector< Bundle* >::iterator iter1 = iter->second.begin();
		for(; iter1 != iter->second.end(); ++iter1)
			Bundle::ObjPool().reclaimObject((*iter1));
	}
...
//-------------------------------------------------------------------------------------
void GhostManager::pushMessage(COMPONENT_ID componentID, Bundle* pBundle)
{
...
//-------------------------------------------------------------------------------------
void GhostManager::pushRouteMessage(ENTITY_ID entityID, COMPONENT_ID componentID, Bundle* pBundle)
{
