...
namespace KBEngine
{
	...
	bool Resmgr::initialize()
	{
		...
		// 获取引擎环境配置
		kb_env_.root			= "E:/Work/FWCycleHero/server";
		kb_env_.res_path		= kb_env_.root + "/res";
		kb_env_.bin_path		= kb_env_.root + "/bin";  
		updatePaths();
	...
	ResourceObjectPtr Resmgr::openResource(const char* res, const char* model, uint32 flags)
	{
		...
		UNORDERED_MAP< std::string, ResourceObjectPtr >::iterator iter = respool_.find(respath);
	...
	{
		KBEngine::thread::ThreadGuard tg(&mutex_); 
		UNORDERED_MAP< std::string, ResourceObjectPtr >::iterator iter = respool_.begin();
