...
Navigation::~Navigation()
{
	...
	UNORDERED_MAP<std::string, NavigationHandlePtr>::iterator iter = navhandles_.begin();
...
{
	KBEngine::thread::ThreadGuard tg(&mutex_); 
	UNORDERED_MAP<std::string, NavigationHandlePtr>::iterator iter = navhandles_.find(name);
