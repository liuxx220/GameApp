	...
	*/
	static bool writeDB(DB_TABLE_OP optype, DBInterface* dbi, DBContext& context)
	{
		...
		else
		{
			...
			UNORDERED_MAP< std::string, std::vector<DBID> > childTableDBIDs;
			...
			if(context.dbid > 0)
			{
				...
				for(; iter1 != context.optable.end(); ++iter1)
				{
					...
					UNORDERED_MAP<std::string, std::vector<DBID> >::iterator iter =
				...
				if(childTableDBIDs.size() > 1)
				{
					...
					UNORDERED_MAP< std::string, std::vector<DBID> >::iterator tabiter = childTableDBIDs.begin();
