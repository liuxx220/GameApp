...
bool EntityTableMysql::syncIndexToDB(DBInterface* dbi)
{
	...
	UNORDERED_MAP<std::string, std::string> getkeys;
	...
	for(; iiter != indexs.end(); )
	{
		...
		UNORDERED_MAP<std::string, std::string>::iterator fiter = getkeys.find(itemname);
...
void EntityTableMysql::getWriteSqlItem(DBInterface* dbi, MemoryStream* s, DBContext& context)
{
	...
	SHARED_PTR< DBContext > opTableValBox1Ptr(context1);
	context.optable.push_back(std::pair<std::string/*tableName*/, SHARED_PTR< DBContext > >
...
void EntityTableItemMysql_VECTOR2::getWriteSqlItem(DBInterface* dbi, MemoryStream* s, DBContext& context)
{
	...
	for(ArraySize i=0; i<asize; ++i)
	{
		...
		context.items.push_back(SHARED_PTR<DBContext::DB_ITEM_DATA>(pSotvs));
	}
}
