...
public class UIScrollView : MonoBehaviour
{
	...
	/// <summary>
	/// 是否向顶部靠齐，起始部位有空间时需要消除
	/// </summary>
	public bool restrictToStart = true;
	...
	/// 拖动到最底部并放开时显示
	/// </summary>
	public OnDragNotification onShowMore;
	/// <summary>
	/// 拖动到最顶部并放开时显示
	/// </summary>
	public OnDragNotification onShowLess;
	...
	/// 强制bounds向起始部位靠齐
	/// </summary>
	public bool RestrictToStart (bool instant, bool horizontal, bool vertical)
	{
		if (mPanel == null)
			return false;
		...
		// 根据bounds信息计算出content的rect和panel rect
		Vector4 cr = mPanel.finalClipRegion;

		float offsetX = cr.z * 0.5f;
		float offsetY = cr.w * 0.5f;

		Vector2 minRect = new Vector2(b.min.x, b.min.y);
		Vector2 maxRect = new Vector2(b.max.x, b.max.y);
		...
		// 先计算content size是否全部包含在panel中，如果不能全部包含，则认为无法进行上部贴合
		Vector2 contentSize = new Vector2 (maxRect.x - minRect.x, maxRect.y - minRect.y);
		Vector2 areaSize = new Vector2 (maxArea.x - minArea.x, maxArea.y - minArea.y);
		if (horizontal && contentSize.x > areaSize.x)
			return false;
		if (vertical && contentSize.y > areaSize.y)
			return false;

		// 计算偏差
		Vector3 constraint = Vector3.zero;
		constraint.x = minArea.x - minRect.x;
		constraint.y = maxArea.y - maxRect.y;

		// 下面两种情况均为 超过顶部
		if (horizontal && constraint.x > 0)
			return false;
		if (vertical && constraint.y < 0)
			return false;
	...
	public void Press (bool pressed)
	{
		...
		if (enabled && NGUITools.GetActive(gameObject))
		{
			...
			{
				bool haveRestrictStart = false;
				// 想start位置进行move
				if (restrictToStart && mPanel.clipping != UIDrawCall.Clipping.None)
				{
					haveRestrictStart = RestrictToStart (dragEffect == DragEffect.None, canMoveHorizontally, canMoveVertically);
				}

				bool haveRestrictPanel = false;
				if (!haveRestrictStart && restrictWithinPanel && mPanel.clipping != UIDrawCall.Clipping.None)
					haveRestrictPanel = RestrictWithinBounds(dragEffect == DragEffect.None, canMoveHorizontally, canMoveVertically);

				if (mDragStarted && onDragFinished != null) onDragFinished();
				if (!mShouldMove && onStoppedMoving != null)
					onStoppedMoving();

				// 如果被限制在bounds，并且检测出需要显示
				if (haveRestrictPanel && onShowMore != null && CheckShowMore ())
					onShowMore ();

				if (haveRestrictPanel && onShowLess != null && CheckShowLess ())
					onShowLess ();
		...
		if (!mPressed)
		{
			if (mMomentum.magnitude > 0.0001f || mScroll != 0f)
			{
				...
				if (restrictToStart && mPanel.clipping != UIDrawCall.Clipping.None)
				{
					...
					{
						haveRestrictStart = RestrictToStart(false, canMoveHorizontally, canMoveVertically);
					}
				}
	...
	/// <summary>
	/// 通过拖动动量计算下一帧的运动方向
	/// 根据运动方向和scroll的方向判断是否到最底部
	/// </summary>
	/// <returns><c>true</c>, if show more was checked, <c>false</c> otherwise.</returns>
	private bool CheckShowMore()
	{
		...
		if (m.magnitude > 0.0001f || mScroll != 0f) {
			if (movement == Movement.Horizontal) {
				m -= mTrans.TransformDirection (new Vector3 (mScroll * 0.05f, 0f, 0f));
			} else if (movement == Movement.Vertical) {
				m -= mTrans.TransformDirection (new Vector3 (0f, mScroll * 0.05f, 0f));
			} else if (movement == Movement.Unrestricted) {
				m -= mTrans.TransformDirection (new Vector3 (mScroll * 0.05f, mScroll * 0.05f, 0f));
			} else {
				m -= mTrans.TransformDirection (new Vector3 (
					mScroll * customMovement.x * 0.05f,
					mScroll * customMovement.y * 0.05f, 0f));
			}

			// Move the scroll view
			Vector3 offset = NGUIMath.SpringDampen (ref m, dampenStrength, delta);

			offset = mTrans.InverseTransformPoint(offset);
			offset = offset - mTrans.InverseTransformPoint(Vector3.zero);;
			...
			// Debug.Log ("Check: " + offset.ToString () + "      " + showMore.ToString());
	...
	/// <summary>
	/// 通过拖动动量计算下一帧的运动方向
	/// 根据运动方向和scroll的方向判断是否到最顶部
	/// </summary>
	/// <returns><c>true</c>, if show more was checked, <c>false</c> otherwise.</returns>
	private bool CheckShowLess()
	{
