...
public class UIBasicSpriteEditor : UIWidgetInspector
{
	...
	protected override void DrawCustomProperties ()
	{
		...
		{
			NGUIEditorTools.DrawProperty("Flip", serializedObject, "mFlip");
			SerializedProperty sp_dir = NGUIEditorTools.DrawProperty("Fill Dir", serializedObject, "mFillDirection", GUILayout.MinWidth(20f));

			UISprite.FillDirection dir = (UIBasicSprite.FillDirection)sp_dir.intValue;

			if (dir == UIBasicSprite.FillDirection.RadialMuti) {
				
				GUILayout.BeginHorizontal();
				GUILayout.Space(4f);
				NGUIEditorTools.DrawPropertyList("Sprite Data", serializedObject, "spriteData", new string[]{"color", "rate"}, GUILayout.MinWidth(20f));
				GUILayout.Space(4f);
