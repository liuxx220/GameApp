...
public abstract class UIBasicSprite : UIWidget
{
	...
	[HideInInspector][SerializeField] protected List<SpriteData> spriteData;
	...
	public float fillAmount
	{
		...
		set
		{
			...
			if (mFillAmount != val)
			{
				if (fillDirection == FillDirection.RadialMuti) {
					mFillAmount = 0.5f;
				} else {
					mFillAmount = val;
				}
...
#region Fill Functions
	...
	void FilledFill (BetterList<Vector3> verts, BetterList<Vector2> uvs, BetterList<Color32> cols)
	{
		...
		if (mFillAmount < 1f)
		{
			...
			if (mFillDirection == FillDirection.RadialCircular) 
			{
				//双向延展
				for (int corner = 0; corner < 4; ++corner)
				{
					...
					float val = corner == 1 || corner == 2 
						? mInvert ? mFillAmount * 2f : mFillAmount * 2f - 1f
						: mInvert ? mFillAmount * 2f - 1f : mFillAmount * 2f;

					if (RadialCut(mTempPos, mTempUVs, Mathf.Clamp01(val), mInvert ? corner > 1 : corner < 2, NGUIMath.RepeatIndex(corner + 2, 4)))
					{
						for (int i = 0; i < 4; ++i)
						{
							verts.Add(mTempPos[i]);
			...
			if (mFillDirection == FillDirection.RadialMuti) 
			{
				//最少要有2个点
				if (spriteData == null || spriteData.Count <= 0)
					return;
				//计算角度
				float startAngle = 1f - spriteData[0].rate * 0.5f;

				for (int i = spriteData.Count-1; i != 0; --i) 
				{
					DrawCircle (verts, uvs, cols, mTempPos, mTempUVs, startAngle, i, spriteData[i].color);

					startAngle -= spriteData[i].rate;
				}

				DrawCircle (verts, uvs, cols, mTempPos, mTempUVs, spriteData[0].rate, 0, spriteData[0].color);
				return;
			}
		}
...
//		Color32 c = drawingColor;
	...
	public void AddSpriteData(List<SpriteData> sp)
	{
		if (sp == null)
			return;
	...
	/// <param name="sp">Sp.</param>
	public void AddSpriteData(int idx, SpriteData sp)
	{
		if (spriteData == null)
			spriteData = new List<SpriteData> ();

		spriteData.Insert (idx, sp);
		fillAmount = 0.5f;
		mChanged = true;
	}
